...
def saveAns(request):
    ...
    for x in myvar:
        print(x.qno,x.ans)
    responce=render(request,'paper/test.html',{'user':User,'test':t,'qnow':t.questions[qno+1],'anow':t.ans[qno+1],'marks':t.marks[qno+1]})
...
def submitPaper(request):
    ...
    tlnew=models.TestLog(test_id=test_id,file_name=file_name)
    tlnew.save()
